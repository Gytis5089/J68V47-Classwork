...
public class Directions {
    public static void main(String[] args) {
        ...
        String toAdd = "After turning onto Elm Street, walk past the grocery store on your right.";
        int location = 4;

        String[] directionsButBetter = new String[directions.length + 1];

        for (int count = 0; count < location; count++) {
            directionsButBetter[count] = directions[count];
        }

        directionsButBetter[location] = toAdd;

        for (int count = location + 1; count < directionsButBetter.length; count++) {
            directionsButBetter[count] = directions[count-1];
        }
